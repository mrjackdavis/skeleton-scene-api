	...
	return self.GetRequest(sceneID)
		.then(function(requestedScene){
			var newScene = {
				...
				requestedAt:new Date(requestedScene.createdAt),
...
SceneStore.prototype.GetScenes = function(size,page) {
	var limit = size || 25;
	var offset = limit * page;
	return this._Toast.findAll({limit:limit,offset:offset,order: [['completedAt', 'DESC']]}).then(function(toasts){
...
function normaliseScene(scene){
	var normalisedObj = scene.dataValues;
	...
	normalisedObj.completedAt = scene.completedAt.getTime();
	normalisedObj.requestedAt = scene.requestedAt.getTime();
	// console.log(normalisedObj);
