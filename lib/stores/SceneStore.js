...
SceneStore.prototype.AddProcessToScene = function(scene,newProcess) {
	...
	var params = {
		...
		Key:{
			...
			dateCreated: { N:scene.dateCreated.valueOf().toString() }
		},
		ExpressionAttributeNames:{
			'pr':'scene.processes'
		},
		ExpressionAttributeValues:{
			':newProcess':{L:[{M:dynoEncode(newProcess)}]}
		},
		UpdateExpression:'SET processes = :newProcess',
	...
	return new Promise(function(resolve,reject){
		self.db.updateItem(params,function(err,data){
			if(err){
				reject(err);
			}else{
				console.log(data);
				resolve(dynoDecode(data.Attributes));
			}
		})
	});
};
